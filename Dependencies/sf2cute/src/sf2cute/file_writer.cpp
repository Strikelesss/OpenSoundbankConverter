...
namespace sf2cute {
	...
	void SoundFontWriter::Write(const std::string& filename) const
	{
		std::ofstream out;
	...
	void SoundFontWriter::Write(std::ostream& out) const
	{
		RIFF riff("sfbk");
	...
	/// Make an INFO chunk.
	std::unique_ptr<RIFFChunkInterface> SoundFontWriter::MakeInfoListChunk() const
	{
		auto info = std::make_unique<RIFFListChunk>("INFO");
	...
	/// Make a sdta chunk.
	std::unique_ptr<RIFFChunkInterface> SoundFontWriter::MakeSdtaListChunk() const
	{
		auto sdta = std::make_unique<RIFFListChunk>("sdta");
		sdta->AddSubchunk(std::make_unique<SFRIFFSmplChunk>(file().samples()));
		return std::move(sdta);
	}

	/// Make a pdta chunk.
	std::unique_ptr<RIFFChunkInterface> SoundFontWriter::MakePdtaListChunk() const
	{
	  // Constructs a map for indexing each instruments.
		...
		auto pdta = std::make_unique<RIFFListChunk>("pdta");
	...
	std::unique_ptr<RIFFChunkInterface> SoundFontWriter::MakeVersionChunk(std::string name, SFVersionTag version) {
		...
		WriteInt16L(data.data(), version.major_version);
